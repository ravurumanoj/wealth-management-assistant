...
Provides a thin, fail-safe wrapper around Google's text-embedding model plus
small numpy helpers used to build an *in-memory* (temporary) similarity index
from rows fetched out of the memory database.

Design goals
------------
* **Graceful degradation** — every public function returns ``None`` / empty on
  any failure (missing API key, network error, etc.) so callers can transparently
  fall back to keyword matching. Memory must never break the chat flow.
* **No external vector store** — embeddings are computed here and similarity is
  done with numpy. Vectors are persisted as JSON text alongside each memory row;
  at read time we stack them into a temporary numpy matrix and cosine-rank.
* **Singleton client** — one embeddings client is shared process-wide.
"""
