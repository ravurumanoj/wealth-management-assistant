...
"""Repository for long-term memory (episodic, semantic, procedural).

Storage model
-------------
Every record is scoped by ``client_id``. Three memory types map to three tables:

* **episodic**   — one row per completed Q&A turn (``episodic_memory``).
* **semantic**   — durable, deduplicated facts (``semantic_memory``).
* **procedural** — behavioural patterns with a frequency counter
  (``procedural_memory``).

Deduplication for semantic/procedural memory is enforced at the database level
via a SHA-256 hash column + unique constraint on ``(client_id, *_hash)``, which
is race-safe across processes (unlike the old in-memory JSON dedup).

Per-client caps bound table growth; the oldest / least-frequent rows are evicted
after each insert.
"""
